...
const MovieDetails = () => {
  ...
  const {shows, user, getToken, axios, favouriteMovies, fetchFavouriteMovies, image_base_url} = useAppContext();
    ...
    <div className="flex flex-col md:flex-row gap-8 max-w-6xl mx-auto">
      ...
      <div className="relative flex flex-col gap-3">
        ...
        <div className="flex items-center flex-wrap gap-4 mt-4">
          ...
          <a href="#dateSelect" className="text-sm bg-primary hover:bg-primary-dull px-10 py-3 transition rounded-md font-medium cursor-pointer active:scale-95">Buy Tickets</a>
          <button onClick={handleFavourites} className="bg-gray-700 p-2.5 rounded-full transition cursor-pointer active:scale-95">
            <HeartIcon className={`size-5 ${favouriteMovies.find((movie) => movie._id === id) ? 'fill-primary text-primary' : ""}`}/>
          </button>
        </div>
      </div>
    </div>
      ...
      <div className="flex items-center gap-4 w-max px-4">
        {show.movie.casts.slice(0,12).map((cast, index)=>(
          <div key={index} className="flex flex-col gap-3 items-center text-center ">
            <img src={image_base_url + cast.profile_path} alt="" className="size-20 md:h-20 object-cover aspect-square rounded-full" />
